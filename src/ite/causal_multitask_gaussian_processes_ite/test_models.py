...
def run_experiment(
    fn_data: str, mode: str = "CMGP", test_frac: float = 0.1
) -> Tuple[np.ndarray, np.ndarray]:

    train_data, test_data = sample_IHDP(fn_data, test_frac=test_frac)
    X_train, W_train, Y_train, T_true_train = (
        train_data[0],
        train_data[1],
        train_data[2],
        train_data[6],
...
def main(args: Any, fn_data: str) -> np.ndarray:
    ...
    for _ in range(args.num_exp):
        ...
        pehe_train_curr, pehe_test_curr = run_experiment(
            fn_data, mode=args.mode, test_frac=args.test_frac
        )
        ...
        print(
            "Experiment: %d (train) \tPEHE: %.3f \t--- (test) \tPEHE: %.3f \t---"
            % (_, pehe_train_curr, pehe_test_curr)
        )

        # on purpose results are calculated within the loop so
        # intermediate results become available while the experiment is ongoing
        ...
        results_d["time_exe"] = time_exe  # type: ignore
        if args.o is not None:
            with open(args.o, "w") as fp:
                json.dump(results_d, fp)

    print(f"exe time {time_exe:0.0f}s")
    print(
        "Final results|| Train PEHE = %.3f +/- %.3f --- Test PEHE = %.3f +/- %.3f"
        % (
            mean_confidence_interval(PEHE_train_np)[0],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Causal Multi-task Gaussian Processes")
    ...
    fn_data = f"{utilmlab.get_data_dir()}/ihdp/ihdp_covariates.csv"

    if not os.path.isfile(fn_data):
        print(
            "Error: this implementation requires the IHDP dataset"
