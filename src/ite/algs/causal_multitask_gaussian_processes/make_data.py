...
def draw_ihdp_data(fn_data: str) -> pd.DataFrame:
    ...
    BetaB_c = np.random.choice(
        coeffs_, size=6, replace=True, p=[0.5, 0.125, 0.125, 0.125, 0.125]
    )
    BetaB_d = np.random.choice(
        coeffs_, size=19, replace=True, p=[0.6, 0.1, 0.1, 0.1, 0.1]
    )
    ...
    DatasetX = pd.DataFrame(
        X,
        columns="X1 X2 X3 X4 X5 X6 X7 X8 X9 X10 X11 X12 X13 X14 X15 X16 X17 X18 X19 X20 X21 X22 X23 X24 X25".split(),
    )
...
def sample_IHDP(fn_data: str, test_frac: float = 0.2) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Y_cf_train = np.array(
        Data_train["Treatment"] * Data_train["Y_0"]
        + (1 - Data_train["Treatment"]) * Data_train["Y_1"]
    )
    ...
    Y_cf_test = np.array(
        Data_test["Treatment"] * Data_test["Y_0"]
        + (1 - Data_test["Treatment"]) * Data_test["Y_1"]
    )
